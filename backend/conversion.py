...
def train_and_export_model():
    ...
    trained_model = YOLO(trained_model_path)  # Load the trained model
    trained_model.export(format="onnx", imgsz=640, opset=9)
    # Convert the model to ONNX format
    onnx_model_path = "yolov8n.onnx"

    # Load ONNX model using ONNX GraphSurgeon
    onnx_model = onnx.load(onnx_model_path)
    graph = gs.import_onnx(onnx_model)

    # Rename input for consistency
    for inp in graph.inputs:
        if "images" in inp.name:
            inp.name = "image"

    # Rename output to match NX AI Manager format
    ...
    # Force downgrade to IR version 12 for compatibility
    onnx_model.ir_version = 9
    onnx.save(onnx_model, onnx_model_path)

    # Verify the IR version after modification
    loaded_model = onnx.load(onnx_model_path)
    print(f"Updated IR Version: {loaded_model.ir_version}")  # Should print 12
